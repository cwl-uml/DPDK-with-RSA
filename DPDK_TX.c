...
char pac_cont[PAYLOAD_SIZE]; 
char en_pac_cont[PAYLOAD_SIZE]; 
//==========================================================================================================
...
{
	//==================================================================================================
	int p=2213,q=2663,e=4727,d=176887,n=5893219,yn=5888344,m[1000],c[10000]; //c[10000]stores transmitting ciphertext,m[1000]stores transmitting plaintext
	...
	for(i=0; i<payload_size/2-1; i++) {
		random = rand() % (77-10)+10;
		payload[i] = zimu_biao[random];}
	//sprintf(payload,"kfGSJD7.e");
	//printf("%s\n",payload);
	strcpy(pac_cont,payload);
	
	//=================================================
	strcpy(min_wen,payload);

	for(k=0;k<strlen(min_wen);k++){
		for(j=10;j<79;j++){
			if(min_wen[k]==zimu_biao[j]){
				m[k]=j;}}}//Based on char table, transfer plaintext into numbers
					
	//RSA Encryption
	for(k=0;k<strlen(min_wen);k++){
		c[k]=js_mod(m[k],e,n);}
	for(k=0;k<strlen(min_wen);k++){
		sprintf(ptr,"%s%d", ptr,c[k]);
	}
	strcpy(payload,ptr);
	strcpy(en_pac_cont,ptr);
