...
//Determine whether the public key "e" meets the requirements
int gcd(int n,int b)
{
	int r1=n,r2=b,r;
	while(r2>0){
...
//Caluculate the value of secret key "d"
...
//Check if the value is a prime number, if yes, return 1; otherwise return 0
...
int app_thread(void *arg)
{
	...
	int status;
	//===============================
	int p=2213,q=2663,e=4727,d=176887,n=5893219,yn=2944172,m[1000],c[10000]; //c[10000]stores transmitting ciphertext,m[1000]stores transmitting plaintext
	//int p=7,q=11,e=1,d=1,n=77,yn=60,m[1000],c[10000];
	int k,j;
	char min_wen[1000],re_min_wen[1000]; //min_wen stores plaintexts that need to be encrypted, re_min_wen stores plaintexts that have been decrypted.
	char ctmp[1000],ptr[1000]="";
	
	//Char table used for encryption
	char zimu_biao[79]="$$$$$$$$$$abcdefghijklmnopqrstuvwxyz,ABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789'.?!";
	//===============================
		...
		while(1)
		{
			...
			while(rte_get_tsc_cycles() - p_start < p_ticks)
			{
				...
				for(i=0; i<n_pkts; i++)
				{
					...
					struct udp_hdr *udph = (struct udp_hdr *) (packet + sizeof(struct sniff_ip) + sizeof(struct sniff_ethernet));
					uint8_t * payload_content = (uint8_t *)(packet + sizeof(struct sniff_ip) + sizeof(struct sniff_ethernet) + sizeof(struct udp_hdr));
					
					strcpy(enpayload,payload_content);
					strcpy(min_wen,payload_content);
					
					//RSA Decryption
					for(k=0,j=0;k<strlen(min_wen)/2;k++){
						strncpy(ctmp,min_wen+j,2);
						c[k]=atoi(ctmp);
						m[k]=js_mod(c[k],d,n);
						j=j+2;
					}
		
					for(k=0;k<strlen(min_wen)/2;k++){
						re_min_wen[k]=zimu_biao[m[k]];
					}
			
					for(k=0;k<strlen(min_wen)/2;k++){
						sprintf(ptr,"%s%c",ptr,re_min_wen[k]);
					}
					strcpy(depayload,ptr);
					//printf("%s says: %s\n", msg.name, msg.text);
					strcpy(ptr,"");
					//printf("%s\n",payload);
			...
			printf("Receive from TX: %s\n",enpayload);
			printf("Plaintext: %s\n",depayload);
			printf("===================================\n");
			/*
			printf("lcore %u, received %u packets in %u seconds.\n", lcore_id, total_pkts, total_time_in_sec);
			printf("SrcAddr = %s\n",inet_ntoa(heavy_hitter.flow.ip_src));
			printf("DstAddr = %s\n",inet_ntoa(heavy_hitter.flow.ip_dst));
			printf("Protocol = %d\n",heavy_hitter.flow.ip_p);
			printf("SrcPort = %d\n",ntohs(heavy_hitter.flow.src_port));
			printf("DstPort = %d\n",ntohs(heavy_hitter.flow.dst_port));
			printf("==========================================\n");*/
		}			
